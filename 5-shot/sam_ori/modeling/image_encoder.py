...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if use_abs_pos:
            ...
            self.post_pos_embed = PostPosEmbed(embed_dim=embed_dim, ori_feature_size=1024//16, new_feature_size=256//16) # new to sam
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = F.interpolate(x, (512 + 8, 512 + 8), mode="bilinear", align_corners=False)
        x = self.proj(x)
        # B C H W -> B H W C
        x = x.permute(0, 2, 3, 1)
        return x
